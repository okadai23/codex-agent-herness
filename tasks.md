...
### T05-01: DB schema/migration基盤を実装する ✅ DONE
...
### T05-02: Seed scenario catalogを実装する ✅ DONE
...
### T05-03: E2Eからseed scenarioを利用できるようにする ✅ DONE
...
実装メモ (2026-05-17):
- `template/packages/db` に migration/seed/scripts を追加し、`db:migrate:*` と `db:verify` を実装。
- `template/tests/e2e/support/seedScenario.ts` を追加し、シナリオseedとtest DB resetをE2Eから呼び出し可能化。
- `template/docs/db-seed-scenarios.md` に seed scenario 一覧を追加し、`example.test` ダミーデータ運用を明記。
## Phase 6: Python AI service標準機能

### T06-01: Python service基盤を実装する ✅ DONE
...
### T06-02: Python Clean Architecture境界を実装する ✅ DONE
...
### T06-03: AI prompt lifecycleを実装する ✅ DONE
...
### T06-05: AI safety checksを実装する ✅ DONE
...
実装メモ (2026-05-17, Phase 6):
- `template/pyproject.toml` と `template/uv.lock` を追加し、uv/Ruff/pytest/FastAPI の最小構成を定義。
- `template/services/ai/src` に domain/application/adapters/infrastructure/api を追加し、LLM provider port と fake provider を実装。
- `template/services/ai/tests` に use case / health / architecture boundary の unit test を追加。
- `template/prompts` に frontmatter 付き prompt artifact を追加（owner/version/input-output schema/safety notes）。
- `template/evals/datasets/{golden,redteam}`、`template/scripts/run_evals.py`、`template/policies/*.md` を追加し、PR/nightly suite・PII検査・report生成を実装。

## Phase 7: Docs maintenance実装

### T07-01: doc-indexを実装する ✅ DONE
...
### T07-02: docs verificationを実装する ✅ DONE
...
### T07-03: harness changelog/adoption reportsを実装する ✅ DONE
...
実装メモ (2026-05-17, Phase 7):
- `template/docs/_meta/doc-index.yaml` と `template/scripts/docs/doc-impact.mjs` を追加し、変更ファイルから影響docsを抽出する `pnpm docs:impact` を実装。
- `template/scripts/docs/verify-docs.mjs` を追加し、docs lint / link / secret-like文字列 / stale_after を一括検証する `pnpm docs:verify` を実装。
- `template/docs/harness/changelog.md` と `template/docs/harness/adoptions/README.md` を追加し、`standardctl apply` のadoption reportに変更内容・検証コマンド・未解決リスクを記録するよう拡張。
...
### T06-01: PM向けSkillとAgent/サブエージェントをテンプレートへ統合する ✅ DONE

- Product Manager向けの `pm-prd-kit` スキルを追加（PRDテンプレート、品質チェックリストを同梱）。
- PMオーケストレーターエージェントと3つのサブエージェント（Researcher/Writer/Critic）を追加。
- テンプレート利用者がすぐ使えるよう、ファイル配置を `.agents/skills` と `.codex/agents` に整備。
- 検証: `pytest -q`

### T07-01: Ubuntu/mac向け Agent ツール導入スクリプトを追加する ✅ DONE

- 追加内容: `scripts/install-agent-tooling.sh` を作成し、Ubuntu/macOS を判定して Node.js と各種CLI導入を自動化。
- 対応ツール: `@anthropic-ai/apm`、`@anthropic-ai/claude-code`、`pnpm`、`typescript`、`tsx` を npm グローバル導入可能にした。
- 検証: `bash -n scripts/install-agent-tooling.sh` で構文チェックを実施。

---

## Runtime拡張フォローアップ（v1.1候補）

### T13-01: `agent_runtime` に応じたテンプレート生成分岐を実装する ✅ DONE

- [x] [DONE] `copier.yml` の `_tasks` で `agent_runtime` に応じて `.codex/**` / `.claude.instructions.md` を生成後整理するよう実装。
- [x] [DONE] `tests/standard/test_runtime_matrix.py` で codex/claude/dual の生成分岐を検証。
- [x] [DONE] `docs/agent-runtimes/*` に検証手順と運用方針を記載。

### T13-02: Runtime別ドキュメントを追加する ✅ DONE

- [x] [DONE] `docs/agent-runtimes/codex.md` を追加。
- [x] [DONE] `docs/agent-runtimes/claude.md` を追加。
- [x] [DONE] `docs/agent-runtimes/dual-runtime-policy.md` を追加。

### T13-03: CIをruntime matrix化する ✅ DONE

- [x] [DONE] CIで `pytest` を通じ `doctor --runtime codex/claude` の動作を検証（`tests/standard/test_runtime_matrix.py`）。
- [x] [DONE] CIで `pytest` を通じ `drift --runtime codex/claude` のコマンド分岐を検証。
- [x] [DONE] `docs/agent-runtimes/dual-runtime-policy.md` に修復手順を追記。

### T13-04: APM core package分割計画を実装に落とす ✅ DONE

- [x] [DONE] `packages/shared-core/apm.yml` / `packages/codex-core/apm.yml` / `packages/claude-core/apm.yml` を追加（PoC）。
- [x] [DONE] runtime別責務は `docs/agent-runtimes/dual-runtime-policy.md` に定義。
- [x] [DONE] 分割PoCとして新規APM packageメタデータを作成し、構成検証テストを追加。


### T08-05: セキュリティ懸念洗い出しと修正計画を作成する ✅ DONE

- [x] [DONE] `scripts/install-agent-tooling.sh` / `scripts/standard/standardctl.py` / `template/services/ai/src/api/main.py` / `template/scripts/run_evals.py` を対象に静的レビューを実施。
- [x] [DONE] `docs/security-assessment-2026-05-18.md` にリスクを高・中・低で整理し、優先度付きの実行計画（Phase A/B/C）を作成。
- [x] [DONE] 受け入れ基準（完了定義）を定義し、今後の実装修正に必要な作業単位を明確化。
